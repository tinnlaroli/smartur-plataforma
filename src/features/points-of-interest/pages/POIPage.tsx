            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm" style={{ color: 'var(--color-text-alt)' }}>
                        Lugares turísticos y su nivel de sostenibilidad
                    </p>
                </div>
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border shadow-sm" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)' }}>
                {points.length === 0 && !isLoading ? (
                    <div className="flex h-64 flex-col items-center justify-center gap-3">
                        ...
                        <p className="text-sm font-medium" style={{ color: 'var(--color-text-alt)' }}>No hay POIs registrados</p>
                    </div>
                ) : (
                        ...
                        <table className="min-w-full">
                            <thead style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-alt)' }}>
                                <tr>
                                    <TH>Nombre</TH>
                                    <TH>Descripción</TH>
                                    <TH><span className="flex items-center gap-1.5"><Tag className="h-3.5 w-3.5" />Tipo</span></TH>
                                    <TH><span className="flex items-center gap-1.5"><Leaf className="h-3.5 w-3.5" />Sostenible</span></TH>
                                </tr>
                            </thead>
                            <tbody>
                                {isLoading ? (
                                    ...
                                    >
                                        <td className="px-5 py-3.5">
                                            <p className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>{poi.name}</p>
                                        </td>
                                        ...
                                        <td className="px-5 py-3.5">
                                            {poi.sustainability ? (
                                                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-semibold text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">
                                                    <Leaf className="h-3 w-3" /> Sostenible
                                                </span>
                                            ) : (
                                                <span className="inline-block rounded-full px-2.5 py-0.5 text-xs font-medium"
