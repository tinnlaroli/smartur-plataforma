...
const ImpactBadge = ({ value, type }: { value: string; type: 'env' | 'social' }) => {
    const low  = type === 'env'
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm" style={{ color: 'var(--color-text-alt)' }}>
                        Impacto social y ambiental por actividad económica
                    </p>
                </div>
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border shadow-sm" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)' }}>
                {activities.length === 0 && !isLoading ? (
                    <div className="flex h-64 flex-col items-center justify-center gap-3">
                        ...
                        <p className="text-sm font-medium" style={{ color: 'var(--color-text-alt)' }}>No hay actividades registradas</p>
                    </div>
                ) : (
                        ...
                        <table className="min-w-full">
                            <thead style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-alt)' }}>
                                <tr>
                                    <TH>#</TH>
                                    <TH>Empresa</TH>
                                    <TH><span className="flex items-center gap-1.5"><DollarSign className="h-3.5 w-3.5" />Producción</span></TH>
                                ...
                                {isLoading ? (
                                    ...
                                    >
                                        ...
                                        <td className="px-5 py-3.5 text-sm font-medium text-emerald-600 dark:text-emerald-400">
                                            ${Number(activity.production_value).toLocaleString('es-MX')}
                                        </td>
                                        <td className="px-5 py-3.5">
                                            <ImpactBadge value={activity.environmental_impact} type="env" />
                                        </td>
                                        <td className="px-5 py-3.5">
                                            <ImpactBadge value={activity.social_impact} type="social" />
                                        </td>
                                    </motion.tr>
                                ))}
                            </tbody>
